    ...
    <div class="page-cate">
        ...
        <div class="app-container">
            ...
            >
                ...
                <el-table-column label="操作" v-if="roleid !== 5">
                    <template slot-scope="scope">
                        <el-button
                            size="mini"
                            type="primary"
...
export default {
    ...
    methods: {
        ...
        onSearch() {
            if (this.searchData) {
                let cateData = this.categorys.filter(item => {
                    // 判断indexOf
                    return item.catename.toString().indexOf(this.searchData) >= 0;
                });
                if (cateData.length === 0) {
                    this.categorys = this.categorys.filter(item => {
                        if (item.children) {
                            item.children = item.children.filter(res => {
                                return res.catename.toString().indexOf(this.searchData) >= 0;
                            })
                            return item.children.length > 0;
                        }
                    });
                } else {
                    this.categorys = cateData;
                }
            } else {
                this.getCateData();
            }
